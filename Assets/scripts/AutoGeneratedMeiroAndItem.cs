...
{
    [Tooltip("配列の一番最後を取得するため、確率の最低値、スタートのZ軸の座標、棒倒しの際に上下左右の座標を見る際、に使う変数")]
    const int _one = 1;
        ...
        for (_column = _two; _column < _fieldz - _one; _column += _two)
        {
            for (_row = _two; _row < _fieldx - _one; _row += _two)
            {
                ...
                if (_probability <= 3)
                {
                    ...
                    else if (field[_column - _one, _row] == false)//壁があったら
                    {
                        _row -= _two; //戻ってやり直す
                    }
                }
                if (_probability >= 4 && _probability <= 6)
                {
                    ...
                    else if (field[_column + _one, _row] == false)
                    {
                        _row -= _two;
                    }
                }
    ...
    void StartAndGoalSet(bool[,] field)
    {
        _goalX = Random.Range(_one, _fieldx - _two);
        if (field[_fieldz - _two, _goalX] == true)//ゴール前が壁の場合
        {
            if (_goalX >= _fieldx / _two)//壁の座標は一つ飛ばしでしか置けないので、座標をずらせばゴール前は壁ではない。
            {
                _goalX--;                
            }
            else
